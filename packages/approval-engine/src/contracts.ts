...
export const ApprovalContinuationSchema = z
  .object({
    conversationId: z.string().min(1).max(200),
    sessionId: z.string().min(1).max(200).nullable(),
    inboundMessageId: z.string().min(1).max(200),
    traceId: z
      .string()
      .regex(/^[0-9a-f]{32}$/)
      .optional()
...
export const ApprovalRequestSchema = z
  .object({
    ...
    /**
     * Durable, tenant-bound pointer used to resume a controlled runtime turn
     * after a decision. It is metadata, never an executable effect payload.
     */
    continuation: ApprovalContinuationSchema.optional()
  })
  .strict()
