...
describeWithPostgres('durable runtime approval authority (PROD-04)', () => {
  ...
  it('ships the continuation pointer and trace migration without weakening RLS', async () => {
    const migration = await readPostgresMigrationSql(
      '0016_runtime_continuation_trace'
    )

    expect(migration).toContain('ADD COLUMN IF NOT EXISTS runtime_approval_id')
    expect(migration).toContain('ADD COLUMN IF NOT EXISTS runtime_trace_id')
    expect(migration).toContain("'waiting_approval'")
    expect(migration).toContain('ADD COLUMN IF NOT EXISTS trace_id')
    expect(migration).toContain('continuation_payload jsonb')
    expect(migration).toContain('uq_runtime_approvals_continuation_message')
  })

  it('atomically commits a decision, continuation outbox event and audit', async () => {
    const hint = `atomic_${randomBytes(4).toString('hex')}`
    ...
    const result = await authority.decideAndEnqueueContinuation({
      tenantId: tenantA,
    ...
    expect(result.continuationEvent).toMatchObject({
      tenantId: tenantA,
    ...
    const persisted = await pool.query<{
      ...
      `SELECT
         (SELECT status FROM runtime_approvals WHERE tenant_id = $1 AND approval_id = $2) AS approval_status,
         (SELECT status FROM outbox_events WHERE tenant_id = $1 AND idempotency_key = $3) AS outbox_status,
         (SELECT count(*)::text FROM audit_events WHERE tenant_id = $1 AND correlation_id = $4) AS audit_count`,
      [
        tenantA,
        record.approvalId,
        `runtime-approval-continuation:${record.approvalId}`,
  ...
  it('rolls the decision back when the continuation enqueue fails', async () => {
    const hint = `rollback_${randomBytes(4).toString('hex')}`
    const record = await authority.request({
      ...requestInput(tenantPending, hint),
      continuation: {
        conversationId: `conv_${hint}`,
        sessionId: `sess_${hint}`,
        inboundMessageId: `msg_${hint}`,
    ...
    const triggerName = `reject_runtime_continuation_${Date.now()}`
    await pool.query(
      `CREATE FUNCTION ${triggerName}() RETURNS trigger LANGUAGE plpgsql AS $$ BEGIN RAISE EXCEPTION 'synthetic continuation enqueue failure'; END $$`
    )
    await pool.query(
    ...
    try {
      ...
      await pool.query(`DROP TRIGGER ${triggerName} ON outbox_events`)
      await pool.query(`DROP FUNCTION ${triggerName}()`)
    }
    await expect(
      authority.get(tenantPending, record.approvalId)
    ...
    const outbox = await pool.query<{ count: string }>(
      ...
      [tenantPending, `runtime-approval-continuation:${record.approvalId}`]
    )
