...
describe('conversation repository behavior', () => {
  ...
  it('persists an approval wait and allows exactly one terminal continuation', () => {
    const { repository } = fixture()
    const created = createConversation(repository)
    const approvalId = 'appr_00000000-0000-4000-8000-000000000a01'
    ...
    const waiting = repository.findInboundRuntimeContext(
      tenantA,
      created.conversation.id,
      created.session.id,
      created.message.id
    )
    expect(waiting?.message).toMatchObject({
    ...
    expect(
      repository.markInboundRuntimeWaitingForApproval(
        created.message.id,
        tenantA,
        'appr_00000000-0000-4000-8000-000000000a02'
      )
    ).toBe(false)
    expect(
      repository.markInboundRuntimeCompleted(created.message.id, tenantA)
    ...
    ).toMatchObject({ runtimeStatus: 'completed', runtimeTraceId: traceId })
    expect(
      repository.findInboundRuntimeContext(
        tenantA,
        created.conversation.id,
