...
describe('AAA-19 worker observability', () => {
  ...
  it('exports governed runtime spans and metrics through the worker sink', () => {
    const lines: string[] = []
    const workerTelemetry = createJsonWorkerTelemetry({
      clock: () => new Date('2026-09-13T00:00:00.000Z'),
      write: (line) => lines.push(line)
    })
    const runtimeTelemetry = createRuntimeTelemetrySink(workerTelemetry)

    runtimeTelemetry
      .startSpan('agent.turn', {
        correlationId: 'corr_00000000-0000-4000-8000-000000000190',
        capability: 'schedule.read'
      })
      .end()
    runtimeTelemetry.recordMetric('agent_runs_total', 1, {
      outcome: 'executed',
      capability: 'schedule.read',
    ...
    expect(JSON.parse(lines[1] as string)).toMatchObject({
      ...
      value: 1,
      attributes: { outcome: 'executed', capability: 'schedule.read' }
    })
  })
})
