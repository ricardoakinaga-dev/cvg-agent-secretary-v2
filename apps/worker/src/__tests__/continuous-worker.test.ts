...
describe('AAA-19 continuous worker shutdown and lease safety', () => {
  ...
  it('does not mutate a bounded SIGTERM lease while the handler is still running', async () => {
    const db = new InMemoryDatabase()
    const adapter = new OutboxRepository(db, {
      retryBaseMs: 5,
      retryMaxMs: 5
    ...
    expect(stopped).toMatchObject({ drained: false, released: 0 })
    expect(adapter.findById(event.id, tenantId)).toMatchObject({
      status: 'processing',
      leaseOwner: 'worker-release-190-a'
    })
  ...
  it('does not start a controlled effect when the initial heartbeat fails', async () => {
    const base = new OutboxRepository(new InMemoryDatabase(), {
      retryBaseMs: 5,
      retryMaxMs: 5
    })
    enqueueInbound(base, 'continuous-heartbeat-failure-190')
    const handler = vi.fn()
    const adapter: DurableOutboxAdapter = {
      enqueue: (input, tx) => base.enqueue(input, tx),
      claimNext: (input) => base.claimNext(input),
      heartbeatClaim: () => {
        throw new Error('synthetic heartbeat transport failure')
      },
    ...
    const worker = createContinuousWorker({
      tenantId,
      workerId: 'worker-heartbeat-failure-190',
      adapter,
  ...
  it('backs off and recovers when the claim boundary fails', async () => {
    ...
    const adapter: DurableOutboxAdapter = {
      ...
      heartbeatClaim: (input) => base.heartbeatClaim(input),
      ack: (input) => base.ack(input),
      fail: (input) => base.fail(input),
      requeueDeadLetter: (input) => base.requeueDeadLetter(input)
    }
