...
function kernelWorkerEnv(
  databaseUrl: string,
  schemaName: string,
  workerId: string
): NodeJS.ProcessEnv {
  return {
    ...workerEnv(databaseUrl, schemaName, workerId),
    CVG_WORKER_RUNTIME: KERNEL_WORKER_RUNTIME,
    CVG_WORKER_AGENT_ID: 'agent_00000000-0000-4000-8000-000000000192'
  }
}
...
describe('AAA-19 continuous PostgreSQL worker', () => {
  ...
  itWithPostgres(
    ...
    async () => {
      ...
      try {
        ...
        expect(boundedStop).toMatchObject({ drained: false, released: 0 })
        const released = await repository.findOutboxById(tenantId, target.id)
        expect(released).toMatchObject({
          status: 'processing',
          leaseOwner: expect.stringContaining('worker-pg-continuous-192-e')
        })
  ...
  itWithPostgres(
    'composes durable approval and journal sweeps for the kernel worker',
    async () => {
      const databaseUrl = testDatabaseUrl as string
      const schemaName = `cvg_worker_sweep_${Date.now()}_${randomBytes(3).toString('hex')}`
      const migrationClient = new Client({ connectionString: databaseUrl })
      const recording = createRecordingTelemetry()
      ...
      try {
        ...
        runtime = createPostgresContinuousWorker(
          kernelWorkerEnv(
            databaseUrl,
            schemaName,
            'worker-pg-sweep-kernel-192'
          ),
          { telemetry: recording.telemetry, tuning }
        )
        runtime.worker.start()

        await vi.waitFor(
          () => {
            expect(recording.logs).toContain('worker.sweep_started')
            expect(recording.logs).toContain('worker.sweep_tick')
          },
          { timeout: 10_000, interval: 25 }
        )
        expect(recording.logs).not.toContain('worker.sweeps_disabled')
      } finally {
