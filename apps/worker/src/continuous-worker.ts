...
const SHUTDOWN_UNSTARTED_CLAIM_ERROR = Object.assign(
  new Error('Worker shutdown discarded a claim before dispatch'),
  { code: 'worker_shutdown_discard_claim' }
)

const LEASE_LOST_BEFORE_EFFECT_ERROR = Object.assign(
  new Error('Worker lease was lost before the controlled effect started'),
  { code: 'worker_lease_lost_before_effect' }
)
 ...
 * stops the lease heartbeat so another worker can retry the same idempotency
 * key after the durable lease expires.
 * Unknown event types and handler failures flow through the repository retry
 * / dead-letter policy and never block the queue. A bounded shutdown never
 * mutates an event that is still executing: its heartbeat is stopped and the
 * durable lease is allowed to expire for a safe retry.
 */
export function createContinuousWorker(
  ...
  const inFlight = new Map<
    Promise<void>,
    { event: OutboxEventRecord; stopHeartbeat: () => void }
  >()
  ...
  function dispatchClaimed(event: OutboxEventRecord): void {
    ...
    const task = (async () => {
      try {
        if (!(await renewLease(event))) {
          throw LEASE_LOST_BEFORE_EFFECT_ERROR
        }
    ...
    inFlight.set(task, { event, stopHeartbeat: heartbeat.stop })
    void task.then(
      () => {
    ...
    stopPromise = (async (): Promise<ContinuousWorkerStopResult> => {
      ...
      if (!settled || inFlight.size > 0) {
        for (const { event, stopHeartbeat } of [...inFlight.values()]) {
          stopHeartbeat()
          telemetry.log(
            'worker.shutdown_deferred_release',
            {
              eventId: event.id,
              workerId,
              leaseAction: 'expire_for_retry',
              operation: 'outbox'
            },
            'warn'
          )
