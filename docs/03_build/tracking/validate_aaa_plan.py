...
local_builds = data.get('localControlledBuilds', [])
assert isinstance(local_builds, list) and local_builds, 'local controlled build record missing'
local_by_task = {item['task']: item for item in local_builds}
assert set(local_by_task) == {'AAA-21'}, 'unexpected locally authorized task'
assert local_by_task['AAA-21']['productionGateChanged'] is False
assert local_by_task['AAA-21']['externalGateChanged'] is False
assert local_by_task['AAA-21']['humanReleaseGateChanged'] is False
assert by_id['AAA-21']['buildAuthorization'] == local_by_task['AAA-21']['authorization']
assert by_id['AAA-21']['contractSha256'] == local_by_task['AAA-21']['specSha256']
assert by_id['AAA-21']['qualityBarSha256'] == local_by_task['AAA-21']['qualityBarSha256']
assert all(
    t['buildAuthorization'] == 'NOT_GRANTED_BY_THIS_PLANNING_DELIVERY'
    for t in tasks
    if t['id'] != 'AAA-21'
)
if args.render:
