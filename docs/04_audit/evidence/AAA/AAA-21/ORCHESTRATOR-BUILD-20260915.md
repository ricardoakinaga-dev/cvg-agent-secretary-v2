...
- Estado: evidência de BUILD; a execução candidate-bound atual registrou `CONDITIONAL_GO / AAA_CONTROLLED` nos manifestos, com produção `NO-GO`.
...
BLOCKED: provider, canal, IdP, RAG institucional, piloto, RPO/RTO físico e signoff humano. Esses bloqueios mantêm o resultado em `CONDITIONAL_GO / AAA_CONTROLLED`; produção permanece `NO-GO`.
